...
<? endif ?>
<form  id="feedback" class="feedback" name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post"
      enctype="multipart/form-data" <?if($arParams["HIDDEN"]=="Y"):?> style="display: none;" <?endif?>>
    <?= bitrix_sessid_post() ?>
    <h3 class="feedback__header">Мы перезвоним</h3>
    ...
    <? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])): ?>
        ...
        <? foreach ($arResult["PROPERTY_LIST"] as $propertyID): ?>
            <div class="feedback__item">
                <label class="feedback__label" for="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID] ?>">
                ...
                switch ($INPUT_TYPE):
                    ...
                    case "N":
                        for ($i = 0; $i < $inputNum; $i++) {
                            ...
                            ?>
                            <input  class="feedback__input" type="text" id="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID] ?>" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="25"
                                    value="<?= $value ?>"/><br/>
    ...
    <input class="feedback__btn colo" type="submit" name="iblock_submit"
           value="<?= GetMessage("IBLOCK_FORM_SUBMIT") ?>"/>
    <? if (strlen($arParams["LIST_URL"]) > 0): ?>
        <input class="feedback__btn colo" type="submit" name="iblock_apply"
